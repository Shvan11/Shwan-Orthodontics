...
- **x-default**: Ensure the `x-default` hreflang tag is always present and points to the English version.
- **Internal Linking**: Never link to `/`. All internal links, including logos and "Back to Site" buttons, MUST use localized paths (e.g., `/${locale}`).
- **Trailing Slashes**: Strictly avoid trailing slashes on all internal links (e.g., `<Link href="/en">` not `/en/`) to prevent duplicate content flags.
- **Sitemap Consistency**: Ensure 1:1 matching between `sitemap.ts` URLs and metadata canonical tags.